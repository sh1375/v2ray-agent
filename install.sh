...
initVar() {
	...
	# v2ray-core、xray-core The path of the configuration file
	configPath=

	# Path of the hysteria configuration file
	hysteriaConfigPath=
	...
	# Does the domain name install a wildcard certificate through dns?
...
checkPortUsedStatus() {
	if lsof -i tcp:80 | grep -q LISTEN; then
		echoContent red "\n ---> Port 80 is inuse, please release the port manually and reinstall\n"
		lsof -i tcp:80 | grep LISTEN
		exit 0
	fi

	if lsof -i tcp:443 | grep -q LISTEN; then
		echoContent red "\n ---> Port 443 is inuse, please release the port manually and reinstall\n"
		lsof -i tcp:80 | grep LISTEN
		exit 0
	fi
}
...
installTools() {
	...
	if ! find /usr/bin /usr/sbin | grep -q -w dig; then
		echoContent green " ---> Installing dig"
	...
	if ! find /usr/bin /usr/sbin | grep -q -w nginx; then
		...
		if [[ ${nginxVersion} -lt 14 ]]; then
			read -r -p "Nginx detected! Read that the current Nginx config doesn't support gRPC, which will cause the installation to fail.Do you want to uninstall Nginx and then reinstall it? (script clean install) [y/n]:" unInstallNginxStatus
			if [[ "${unInstallNginxStatus}" == "y" ]]; then
				${removeType} nginx >/dev/null 2>&1
				echoContent yellow " ---> Nginx Uninstalled"
				echoContent green " --->  Nginx Installed"
	...
	if [[ ! -d "$HOME/.acme.sh" ]] || [[ -d "$HOME/.acme.sh" && -z $(find "$HOME/.acme.sh/acme.sh") ]]; then
		...
		if [[ ! -d "$HOME/.acme.sh" ]] || [[ -z $(find "$HOME/.acme.sh/acme.sh") ]]; then
			...
			echoContent red " 3.For ONLY IPv6 machines, please set up NAT64 and execute the following commands"
...
checkIP() {
	...
	if [[ -z ${localIP} ]] || ! echo "${localIP}" | sed '1{s/[^(]*(//;s/).*//;q}' | grep -q '\.' && ! echo "${localIP}" | sed '1{s/[^(]*(//;s/).*//;q}' | grep -q ':'; then
		...
		echoContent yellow " ---> 4.If a problem with Nginx startup is reported, please manually start nginx to check the error. If you can't handle it yourself, please submit an issue."
		echoContent yellow " ---> 5.Error log: ${localIP}"
		echo
		echoContent skyBlue " ---> If the above settings are correct, please reinstall the script and try again"
		...
		if echo "${localIP}" | awk -F "[,]" '{print $2}' | grep -q "." || echo "${localIP}" | awk -F "[,]" '{print $2}' | grep -q ":"; then
			echoContent red "\n ---> Multiple IPs detected, please make sure that cloudflare's cloud is off (proxy is disabled)"
			echoContent yellow " ---> After disabling the cloud, wait three minutes and try again"
...
selectAcmeInstallSSL() {
	...
	if [[ -n "${customPort}" ]]; then
		...
		read -r -p "Do you want to use DNS to verify for certificate? [y/n]:" installSSLDNSStatus
...
checkCustomPort() {
	if lsof -i "tcp:${customPort}" | grep -q LISTEN; then
		echoContent red "\n ---> ${customPort} port is inuse, please release it manually and re-install the script\n"
		lsof -i tcp:80 | grep LISTEN
		exit 0
	fi
}
...
randomPathFunction() {
	...
	if [[ "${historyPathStatus}" == "y" ]]; then
		...
		echoContent yellow "Please enter a custom path [example: stream], no slashes required, [enter] random path"
...
nginxBlog() {
	...
	if [[ -d "/usr/share/nginx/html" && -f "/usr/share/nginx/html/check" ]]; then
		...
		if [[ "${nginxBlogInstallStatus}" == "y" ]]; then
			...
			echoContent green " ---> Fake website added successfully"
		fi
	else
		randomNum=$((RANDOM % 6 + 1))
		rm -rf /usr/share/nginx/html
...
installCronTLS() {
	...
	echo "20 4 * * * /usr/bin/bash /etc/v2ray-agent/install.sh RenewTLS >> /etc/v2ray-agent/crontab_tls.log 2>&1" >> /etc/v2ray-agent/backup_crontab.cron
...
v2rayVersionManageMenu() {
	...
	echoContent yellow "2.Fallback versions v2ray-core"
	echoContent yellow "3.Stop v2ray-core service"
	echoContent yellow "4.Start v2ray-core service"
...
xrayVersionManageMenu() {
	...
	echoContent yellow "3.Fallback versions Xray-core"
	echoContent yellow "4.Stop Xray-core service"
	echoContent yellow "5.Start Xray-core service"
...
# Custom CDN IP
customCDNIP() {
	echoContent skyBlue "\nProgress $1/${totalProgress} : add CDN custom CNAME"
	echoContent red "\n=============================================================="
	echoContent yellow "# Notes"
	echoContent yellow "\nTutorial address:"
...
updateV2RayAgent() {
	...
	if wget --help | grep -q show-progress; then
		...
		wget -c -q -P /etc/v2ray-agent/ -N --no-check-certificate "https://github.com/sh1375/v2ray-agent/master/install.sh"
	fi
	...
	echoContent green "\n ---> Update completed"
	echoContent yellow " ---> Please manually execute [rose] to open the script"
...
checkLog() {
	...
	if [[ "${logStatus}" == "false" ]]; then
		echoContent yellow "1.Enable access log"
	else
		echoContent yellow "1.Disable the access log"
	fi

	echoContent yellow "2.View access log"
	echoContent yellow "3.View error log"
...
aliasInstall() {
	...
	if [[ -f "$HOME/install.sh" ]] && [[ -d "/etc/v2ray-agent" ]] && grep <"$HOME/install.sh" -q "Author: sh1375"; then
		...
		if [[ "${roseType}" == "true" ]]; then
			echoContent green "Shortcut created successfully, execute [rose] to reopen the script"
		fi
	fi
}
...
ipv6Routing() {
	...
	checkIPv6
	echoContent skyBlue "\nFunction 1/${totalProgress} : IPv6 Manager"
	echoContent red "\n=============================================================="
	echoContent yellow "1.Add Geosite category"
...
btTools() {
	if [[ -z "${configPath}" ]]; then
				echoContent red " ---> Not activated, please use the script to install"
		menu
		exit 0
	fi

	echoContent skyBlue "\nFunction 1/${totalProgress} : bittorrent download manager"
	echoContent red "\n=============================================================="

	if [[ -f ${configPath}09_routing.json ]] && grep -q bittorrent <${configPath}09_routing.json; then
		echoContent yellow "Current Status: Disabled, Users Can't download from bittorrent"
	else
		echoContent yellow "Current status: Enabled"
	fi

	echoContent yellow "1.Enable Download from bittorrent"
	echoContent yellow "2.Disable Download from bittorrent"
	echoContent red "=============================================================="
...
EOF
		...
		echoContent green " ---> BT download successfully Enabled"
	else
		echoContent red " ---> Selection error"
		exit 0
	fi
...
blacklist() {
	...
	if [[ "${blacklistStatus}" == "1" ]]; then
		...
		echoContent yellow "3.Special characters are not allowed, pay attention to the format of commas"
		echoContent yellow "4.Every time you add it, it will be added again, and the last domain name will not be retained"
		echoContent yellow "5.I'll add the Domain name later.\n"
		read -r -p "Please enter the geosite category according to the above examples:" domainList
...
warpRouting() {
	...
	echoContent red "=============================================================="
	#	echoContent yellow "# Notes\n"
	#	echoContent yellow "1.The official warp has bugs after several rounds of testing. Restarting will cause the warp to fail and fail to start, and CPU usage may skyrocket."
	...
	if [[ -z $(which warp-cli) ]]; then
		echo
		read -r -p "WARP is not installed, Install WARP? [y/n]:" installCloudflareWarpStatus
	...
	echoContent red "\n=============================================================="
	echoContent yellow "1.Add geosite category for passing through the WARP"
	echoContent yellow "2.Uninstall WARP"
	echoContent red "=============================================================="
...
dokodemoDoorUnblockStreamingMedia() {
	...
	echoContent red "\n=============================================================="
	echoContent yellow "# Notes"
	echoContent yellow "Any door unlock details, please check this article [https://github.com/sh1375/v2ray-agent/blob/master/documents/netflix/dokodemo-unblock_netflix.md]\n"
