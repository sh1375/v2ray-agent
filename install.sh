...
readInstallType() {
	...
	# 1.检测安装目录
	if [[ -d "/etc/captain-claw" ]]; then
		# 检测安装方式 v2ray-core
		if [[ -d "/etc/captain-claw/v2ray" && -f "/etc/captain-claw/v2ray/v2ray" && -f "/etc/captain-claw/v2ray/v2ctl" ]]; then
			if [[ -d "/etc/captain-claw/v2ray/conf" && -f "/etc/captain-claw/v2ray/conf/02_VLESS_TCP_inbounds.json" ]]; then
				configPath=/etc/captain-claw/v2ray/conf/
				if grep </etc/captain-claw/v2ray/conf/02_VLESS_TCP_inbounds.json -q '"security": "tls"'; then
					# 不带XTLS的v2ray-core
					coreInstallType=2
					ctlPath=/etc/captain-claw/v2ray/v2ctl
				elif grep </etc/captain-claw/v2ray/conf/02_VLESS_TCP_inbounds.json -q '"security": "xtls"'; then
					# 带XTLS的v2ray-core
					ctlPath=/etc/captain-claw/v2ray/v2ctl
					coreInstallType=3
				fi
		...
		if [[ -d "/etc/captain-claw/xray" && -f "/etc/captain-claw/xray/xray" ]]; then
			# 这里检测xray-core
			if [[ -d "/etc/captain-claw/xray/conf" ]] && [[ -f "/etc/captain-claw/xray/conf/02_VLESS_TCP_inbounds.json" || -f "/etc/captain-claw/xray/conf/02_trojan_TCP_inbounds.json" ]]; then
				# xray-core
				configPath=/etc/captain-claw/xray/conf/
				ctlPath=/etc/captain-claw/xray/xray
				coreInstallType=1
			fi
		fi

		if [[ -d "/etc/captain-claw/hysteria" && -f "/etc/captain-claw/hysteria/hysteria" ]]; then
			# 这里检测 hysteria
			if [[ -d "/etc/captain-claw/hysteria/conf" ]] && [[ -f "/etc/captain-claw/hysteria/conf/config.json" ]] && [[ -f "/etc/captain-claw/hysteria/conf/client_network.json" ]]; then
				hysteriaConfigPath=/etc/captain-claw/hysteria/conf/
			fi
		fi
...
cleanUp() {
	if [[ "$1" == "v2rayClean" ]]; then
		rm -rf "$(find /etc/captain-claw/v2ray/* | grep -E '(config_full.json|conf)')"
		handleV2Ray stop >/dev/null
		rm -f /etc/systemd/system/v2ray.service
	elif [[ "$1" == "xrayClean" ]]; then
		rm -rf "$(find /etc/captain-claw/xray/* | grep -E '(config_full.json|conf)')"
		handleXray stop >/dev/null
		rm -f /etc/systemd/system/xray.service

	elif [[ "$1" == "v2rayDel" ]]; then
		rm -rf /etc/captain-claw/v2ray/*

	elif [[ "$1" == "xrayDel" ]]; then
		rm -rf /etc/captain-claw/xray/*
	fi
}
...
# 初始化安装目录
mkdirTools() {
	mkdir -p /etc/captain-claw/tls
	mkdir -p /etc/captain-claw/subscribe
	mkdir -p /etc/captain-claw/subscribe_tmp
	mkdir -p /etc/captain-claw/v2ray/conf
	mkdir -p /etc/captain-claw/v2ray/tmp
	mkdir -p /etc/captain-claw/xray/conf
	mkdir -p /etc/captain-claw/xray/tmp
	mkdir -p /etc/captain-claw/trojan
	mkdir -p /etc/captain-claw/hysteria/conf
...
installTools() {
	...
	${upgrade} >/etc/captain-claw/install.log 2>&1
	if grep <"/etc/captain-claw/install.log" -q "changed"; then
	...
	if [[ ! -d "$HOME/.acme.sh" ]] || [[ -d "$HOME/.acme.sh" && -z $(find "$HOME/.acme.sh/acme.sh") ]]; then
		...
		curl -s https://get.acme.sh | sh -s >/etc/captain-claw/tls/acme.log 2>&1

		if [[ ! -d "$HOME/.acme.sh" ]] || [[ -z $(find "$HOME/.acme.sh/acme.sh") ]]; then
			echoContent red "  acme安装失败--->"
			tail -n 100 /etc/captain-claw/tls/acme.log
...
server {
	...
	location /s/ {
    	...
    	alias /etc/captain-claw/subscribe/;
    }
...
switchSSLType() {
	if [[ -z "${sslType}" ]]; then
		...
		*)
		...
		touch /etc/captain-claw/tls
		echo "${sslType}" >/etc/captain-claw/tls/ssl_type
...
acmeInstallSSL() {
	if [[ "${dnsSSLStatus}" == "true" ]]; then
		...
		sudo "$HOME/.acme.sh/acme.sh" --issue -d "*.${dnsTLSDomain}" -d "${dnsTLSDomain}" --dns --yes-I-know-dns-manual-mode-enough-go-ahead-please -k ec-256 --server "${sslType}" ${installSSLIPv6} 2>&1 | tee -a /etc/captain-claw/tls/acme.log >/dev/null

		local txtValue=
		txtValue=$(tail -n 10 /etc/captain-claw/tls/acme.log | grep "TXT value" | awk -F "'" '{print $2}')
		if [[ -n "${txtValue}" ]]; then
			...
			if [[ "${addDNSTXTRecordStatus}" == "y" ]]; then
				...
				if echo "${txtAnswer}" | grep -q "${txtValue}"; then
					...
					sudo "$HOME/.acme.sh/acme.sh" --renew -d "*.${dnsTLSDomain}" -d "${dnsTLSDomain}" --yes-I-know-dns-manual-mode-enough-go-ahead-please --ecc --server "${sslType}" ${installSSLIPv6} 2>&1 | tee -a /etc/captain-claw/tls/acme.log >/dev/null
				else
		...
		echoContent green " ---> 生成证书中"
		sudo "$HOME/.acme.sh/acme.sh" --issue -d "${tlsDomain}" --standalone -k ec-256 --server "${sslType}" ${installSSLIPv6} 2>&1 | tee -a /etc/captain-claw/tls/acme.log >/dev/null
	fi
}
# 自定义端口
customPortFunction() {
...
installTLS() {
	...
	# 安装tls
	if [[ -f "/etc/captain-claw/tls/${tlsDomain}.crt" && -f "/etc/captain-claw/tls/${tlsDomain}.key" && -n $(cat "/etc/captain-claw/tls/${tlsDomain}.crt") ]] || [[ -d "$HOME/.acme.sh/${tlsDomain}_ecc" && -f "$HOME/.acme.sh/${tlsDomain}_ecc/${tlsDomain}.key" && -f "$HOME/.acme.sh/${tlsDomain}_ecc/${tlsDomain}.cer" ]]; then
		echoContent green " ---> 检测到证书"
		# checkTLStatus
		renewalTLS

		if [[ -z $(find /etc/captain-claw/tls/ -name "${tlsDomain}.crt") ]] || [[ -z $(find /etc/captain-claw/tls/ -name "${tlsDomain}.key") ]] || [[ -z $(cat "/etc/captain-claw/tls/${tlsDomain}.crt") ]]; then
			sudo "$HOME/.acme.sh/acme.sh" --installcert -d "${tlsDomain}" --fullchainpath "/etc/captain-claw/tls/${tlsDomain}.crt" --keypath "/etc/captain-claw/tls/${tlsDomain}.key" --ecc >/dev/null
		else
			echoContent yellow " ---> 如未过期或者自定义证书请选择[n]\n"
			read -r -p "是否重新安装？[y/n]:" reInstallStatus
			if [[ "${reInstallStatus}" == "y" ]]; then
				rm -rf /etc/captain-claw/tls/*
				installTLS "$1"
			fi
		fi
		...
		if [[ "${installDNSACMEStatus}" == "true" ]]; then
			echo
			if [[ -d "$HOME/.acme.sh/*.${dnsTLSDomain}_ecc" && -f "$HOME/.acme.sh/*.${dnsTLSDomain}_ecc/*.${dnsTLSDomain}.key" && -f "$HOME/.acme.sh/*.${dnsTLSDomain}_ecc/*.${dnsTLSDomain}.cer" ]]; then
				sudo "$HOME/.acme.sh/acme.sh" --installcert -d "*.${dnsTLSDomain}" --fullchainpath "/etc/captain-claw/tls/${tlsDomain}.crt" --keypath "/etc/captain-claw/tls/${tlsDomain}.key" --ecc >/dev/null
			fi
		...
		if [[ ! -f "/etc/captain-claw/tls/${tlsDomain}.crt" || ! -f "/etc/captain-claw/tls/${tlsDomain}.key" ]] || [[ -z $(cat "/etc/captain-claw/tls/${tlsDomain}.key") || -z $(cat "/etc/captain-claw/tls/${tlsDomain}.crt") ]]; then
			tail -n 10 /etc/captain-claw/tls/acme.log
			...
			if tail -n 10 /etc/captain-claw/tls/acme.log | grep -q "Could not validate email address as valid"; then
...
updateSELinuxHTTPPortT() {
	...
	$(find /usr/bin /usr/sbin | grep -w journalctl) -xe >/etc/captain-claw/nginx_error.log 2>&1

	if find /usr/bin /usr/sbin | grep -q -w semanage && find /usr/bin /usr/sbin | grep -q -w getenforce && grep -E "31300|31302" </etc/captain-claw/nginx_error.log | grep -q "Permission denied"; then
...
handleNginx() {
	...
	if [[ -z $(pgrep -f "nginx") ]] && [[ "$1" == "start" ]]; then
		systemctl start nginx 2>/etc/captain-claw/nginx_error.log
		...
		if [[ -z $(pgrep -f nginx) ]]; then
			...
			if grep -q "journalctl -xe" </etc/captain-claw/nginx_error.log; then
				updateSELinuxHTTPPortT
			fi
...
installCronTLS() {
	...
	crontab -l >/etc/captain-claw/backup_crontab.cron
	local historyCrontab
	historyCrontab=$(sed '/v2ray-agent/d;/acme.sh/d' /etc/captain-claw/backup_crontab.cron)
	echo "${historyCrontab}" >/etc/captain-claw/backup_crontab.cron
	echo "30 1 * * * /bin/bash /etc/captain-claw/install.sh RenewTLS >> /etc/captain-claw/crontab_tls.log 2>&1" >>/etc/captain-claw/backup_crontab.cron
	crontab /etc/captain-claw/backup_crontab.cron
...
renewalTLS() {
	...
	if [[ -f "/etc/captain-claw/tls/ssl_type" ]]; then
		if grep -q "buypass" <"/etc/captain-claw/tls/ssl_type"; then
			sslRenewalDays=180
		fi
	fi
	if [[ -d "$HOME/.acme.sh/${domain}_ecc" && -f "$HOME/.acme.sh/${domain}_ecc/${domain}.key" && -f "$HOME/.acme.sh/${domain}_ecc/${domain}.cer" ]] || [[ "${installDNSACMEStatus}" == "true" ]]; then
		...
		if [[ ${remainingDays} -le 1 ]]; then
			...
			sudo "$HOME/.acme.sh/acme.sh" --installcert -d "${domain}" --fullchainpath /etc/captain-claw/tls/"${domain}.crt" --keypath /etc/captain-claw/tls/"${domain}.key" --ecc
			reloadCore
			handleNginx start
		else
...
installV2Ray() {
	...
	if [[ "${coreInstallType}" != "2" && "${coreInstallType}" != "3" ]]; then
		...
		if wget --help | grep -q show-progress; then
			wget -c -q --show-progress -P /etc/captain-claw/v2ray/ "https://github.com/v2fly/v2ray-core/releases/download/${version}/${v2rayCoreCPUVendor}.zip"
		else
			wget -c -P /etc/captain-claw/v2ray/ "https://github.com/v2fly/v2ray-core/releases/download/${version}/${v2rayCoreCPUVendor}.zip" >/dev/null 2>&1
		fi

		unzip -o "/etc/captain-claw/v2ray/${v2rayCoreCPUVendor}.zip" -d /etc/captain-claw/v2ray >/dev/null
		rm -rf "/etc/captain-claw/v2ray/${v2rayCoreCPUVendor}.zip"
	else
		if [[ "${selectCoreType}" == "3" ]]; then
			echoContent green " ---> 锁定v2ray-core版本为v4.32.1"
			rm -f /etc/captain-claw/v2ray/v2ray
			rm -f /etc/captain-claw/v2ray/v2ctl
			installV2Ray "$1"
		else
			echoContent green " ---> v2ray-core版本:$(/etc/captain-claw/v2ray/v2ray --version | awk '{print $2}' | head -1)"
...
installHysteria() {
	...
	if [[ -z "${hysteriaConfigPath}" ]]; then
		...
		if wget --help | grep -q show-progress; then
			wget -c -q --show-progress -P /etc/captain-claw/hysteria/ "https://github.com/apernet/hysteria/releases/download/${version}/${hysteriaCoreCPUVendor}"
		else
			wget -c -P /etc/captain-claw/hysteria/ "https://github.com/apernet/hysteria/releases/download/${version}/${hysteriaCoreCPUVendor}" >/dev/null 2>&1
		fi
		mv "/etc/captain-claw/hysteria/${hysteriaCoreCPUVendor}" /etc/captain-claw/hysteria/hysteria
		chmod 655 /etc/captain-claw/hysteria/hysteria
	else
		echoContent green " ---> Hysteria版本:$(/etc/captain-claw/hysteria/hysteria --version | awk '{print $3}')"
		read -r -p "是否更新、升级？[y/n]:" reInstallHysteriaStatus
		if [[ "${reInstallHysteriaStatus}" == "y" ]]; then
			rm -f /etc/captain-claw/hysteria/hysteria
			installHysteria "$1"
		fi
	fi
...
installXray() {
	...
	if [[ "${coreInstallType}" != "1" ]]; then
		...
		if wget --help | grep -q show-progress; then
			wget -c -q --show-progress -P /etc/captain-claw/xray/ "https://github.com/XTLS/Xray-core/releases/download/${version}/${xrayCoreCPUVendor}.zip"
		else
			wget -c -P /etc/captain-claw/xray/ "https://github.com/XTLS/Xray-core/releases/download/${version}/${xrayCoreCPUVendor}.zip" >/dev/null 2>&1
		fi

		unzip -o "/etc/captain-claw/xray/${xrayCoreCPUVendor}.zip" -d /etc/captain-claw/xray >/dev/null
		rm -rf "/etc/captain-claw/xray/${xrayCoreCPUVendor}.zip"
		chmod 655 /etc/captain-claw/xray/xray
	else
		echoContent green " ---> Xray-core版本:$(/etc/captain-claw/xray/xray --version | awk '{print $2}' | head -1)"
		read -r -p "是否更新、升级？[y/n]:" reInstallXrayStatus
		if [[ "${reInstallXrayStatus}" == "y" ]]; then
			rm -f /etc/captain-claw/xray/xray
			installXray "$1"
		fi
	fi
}
...
v2rayVersionManageMenu() {
	...
	if [[ ! -d "/etc/captain-claw/v2ray/" ]]; then
...
xrayVersionManageMenu() {
	...
	if [[ ! -d "/etc/captain-claw/xray/" ]]; then
		echoContent red " ---> 没有检测到安装目录，请执行脚本安装内容"
		menu
		exit 0
	fi
...
updateV2Ray() {
	...
	if [[ -z "${coreInstallType}" ]]; then
		...
		echoContent green " ---> 当前v2ray-core版本:$(/etc/captain-claw/v2ray/v2ray --version | awk '{print $2}' | head -1)"
		...
		if [[ -n "$1" ]]; then
			...
			if [[ "${rollbackV2RayStatus}" == "y" ]]; then
				if [[ "${coreInstallType}" == "2" || "${coreInstallType}" == "3" ]]; then
					...
					echoContent green " ---> 当前Xray-core版本:$(/etc/captain-claw/xray/xray --version | awk '{print $2}' | head -1)"
				fi
		...
		elif [[ "${version}" == "v$(/etc/captain-claw/v2ray/v2ray --version | awk '{print $2}' | head -1)" ]]; then
...
updateXray() {
	...
	if [[ -z "${coreInstallType}" ]]; then
		...
		elif [[ "${version}" == "v$(/etc/captain-claw/xray/xray --version | awk '{print $2}' | head -1)" ]]; then
...
installV2RayService() {
	...
	if [[ -n $(find /bin /usr/bin -name "systemctl") ]]; then
		...
		execStart='/etc/captain-claw/v2ray/v2ray -confdir /etc/captain-claw/v2ray/conf'
...
installHysteriaService() {
	...
	if [[ -n $(find /bin /usr/bin -name "systemctl") ]]; then
		...
		execStart='/etc/captain-claw/hysteria/hysteria --log-level info -c /etc/captain-claw/hysteria/conf/config.json server'
...
installXrayService() {
	...
	if [[ -n $(find /bin /usr/bin -name "systemctl") ]]; then
		...
		execStart='/etc/captain-claw/xray/xray run -confdir /etc/captain-claw/xray/conf'
...
handleV2Ray() {
	...
	if [[ "$1" == "start" ]]; then
		if [[ -n $(pgrep -f "v2ray/v2ray") ]]; then
			...
			echoContent red "请手动执行【/etc/captain-claw/v2ray/v2ray -confdir /etc/captain-claw/v2ray/conf】，查看错误日志"
			exit 0
		fi
	elif [[ "$1" == "stop" ]]; then
...
handleHysteria() {
	...
	if [[ "$1" == "start" ]]; then
		if [[ -n $(pgrep -f "hysteria/hysteria") ]]; then
			...
			echoContent red "请手动执行【/etc/captain-claw/hysteria/hysteria --log-level debug -c /etc/captain-claw/hysteria/conf/config.json server】，查看错误日志"
			exit 0
		fi
	elif [[ "$1" == "stop" ]]; then
...
handleXray() {
	...
	if [[ "$1" == "start" ]]; then
		if [[ -n $(pgrep -f "xray/xray") ]]; then
			...
			echoContent red "请手动执行【/etc/captain-claw/xray/xray -confdir /etc/captain-claw/xray/conf】，查看错误日志"
			exit 0
		fi
	elif [[ "$1" == "stop" ]]; then
...
initHysteriaNetwork() {
	...
	cat <<EOF >/etc/captain-claw/hysteria/conf/client_network.json
{
...
initHysteriaConfig() {
	...
	cat <<EOF >/etc/captain-claw/hysteria/conf/config.json
{
	"listen": ":${hysteriaPort}",
	"protocol": "${hysteriaProtocol}",
	"disable_udp": false,
	"cert": "/etc/captain-claw/tls/${currentHost}.crt",
	"key": "/etc/captain-claw/tls/${currentHost}.key",
	"auth": {
		"mode": "passwords",
		"config": []
	},
...
EOF
	...
	addClientsHysteria "/etc/captain-claw/hysteria/conf/config.json" true
}
...
initV2RayConfig() {
	...
	if [[ -n "${currentUUID}" && -z "${uuid}" ]]; then
		...
		if [[ "${historyUUIDStatus}" == "y" ]]; then
			...
			uuid=$(/etc/captain-claw/v2ray/v2ctl uuid)
		fi
	elif [[ -z "${uuid}" ]]; then
		uuid=$(/etc/captain-claw/v2ray/v2ctl uuid)
	fi
	...
	movePreviousConfig
	# log
	cat <<EOF >/etc/captain-claw/v2ray/conf/00_log.json
{
  "log": {
    "error": "/etc/captain-claw/v2ray/error.log",
    "loglevel": "warning"
  }
}
EOF
	# outbounds
	if [[ -n "${pingIPv6}" ]]; then
		cat <<EOF >/etc/captain-claw/v2ray/conf/10_ipv6_outbounds.json
{
    "outbounds": [
        {
          "protocol": "freedom",
...
EOF
		...
		cat <<EOF >/etc/captain-claw/v2ray/conf/10_ipv4_outbounds.json
{
    "outbounds":[
        {
            "protocol":"freedom",
...
EOF
	...
	# dns
	cat <<EOF >/etc/captain-claw/v2ray/conf/11_dns.json
{
    "dns": {
        "servers": [
          "localhost"
...
EOF
	...
	if echo "${selectCustomInstallType}" | grep -q 4 || [[ "$1" == "all" ]]; then
		...
		cat <<EOF >/etc/captain-claw/v2ray/conf/04_trojan_TCP_inbounds.json
{
"inbounds":[
	{
	  "port": 31296,
...
EOF
		addClients "/etc/captain-claw/v2ray/conf/04_trojan_TCP_inbounds.json" "${addClientsStatus}"
	fi
	...
	if echo "${selectCustomInstallType}" | grep -q 1 || [[ "$1" == "all" ]]; then
		...
		cat <<EOF >/etc/captain-claw/v2ray/conf/03_VLESS_WS_inbounds.json
{
"inbounds":[
    {
	  "port": 31297,
...
EOF
		addClients "/etc/captain-claw/v2ray/conf/03_VLESS_WS_inbounds.json" "${addClientsStatus}"
	fi
	...
	if echo "${selectCustomInstallType}" | grep -q 2 || [[ "$1" == "all" ]]; then
		...
		cat <<EOF >/etc/captain-claw/v2ray/conf/04_trojan_gRPC_inbounds.json
{
    "inbounds": [
        {
            "port": 31304,
...
EOF
		addClients "/etc/captain-claw/v2ray/conf/04_trojan_gRPC_inbounds.json" "${addClientsStatus}"
	fi
	...
	if echo "${selectCustomInstallType}" | grep -q 3 || [[ "$1" == "all" ]]; then
		...
		cat <<EOF >/etc/captain-claw/v2ray/conf/05_VMess_WS_inbounds.json
{
"inbounds":[
{
  "listen": "127.0.0.1",
...
EOF
		addClients "/etc/captain-claw/v2ray/conf/05_VMess_WS_inbounds.json" "${addClientsStatus}"
	fi

	if echo "${selectCustomInstallType}" | grep -q 5 || [[ "$1" == "all" ]]; then
		getClients "${configPath}../tmp/06_VLESS_gRPC_inbounds.json" "${addClientsStatus}"
		cat <<EOF >/etc/captain-claw/v2ray/conf/06_VLESS_gRPC_inbounds.json
{
    "inbounds":[
    {
        "port": 31301,
...
EOF
		addClients "/etc/captain-claw/v2ray/conf/06_VLESS_gRPC_inbounds.json" "${addClientsStatus}"
	fi
	...
	cat <<EOF >/etc/captain-claw/v2ray/conf/02_VLESS_TCP_inbounds.json
{
"inbounds":[
{
  "port": ${defaultPort},
  ...
  "streamSettings": {
    ...
    "tlsSettings": {
      ...
      "certificates": [
        {
          "certificateFile": "/etc/captain-claw/tls/${domain}.crt",
          "keyFile": "/etc/captain-claw/tls/${domain}.key",
          "ocspStapling": 3600,
          "usage":"encipherment"
        }
      ]
...
EOF
	addClients "/etc/captain-claw/v2ray/conf/02_VLESS_TCP_inbounds.json" "${addClientsStatus}"
...
initXrayConfig() {
	...
	if [[ -z "${uuid}" ]]; then
		...
		if [[ -n ${customUUID} ]]; then
			...
			uuid=$(/etc/captain-claw/xray/xray uuid)
		fi
	...
	# log
	cat <<EOF >/etc/captain-claw/xray/conf/00_log.json
{
  "log": {
    "error": "/etc/captain-claw/xray/error.log",
    "loglevel": "warning"
  }
}
EOF
	...
	# outbounds
	if [[ -n "${pingIPv6}" ]]; then
		cat <<EOF >/etc/captain-claw/xray/conf/10_ipv6_outbounds.json
{
    "outbounds": [
        {
          "protocol": "freedom",
...
EOF
		...
		cat <<EOF >/etc/captain-claw/xray/conf/10_ipv4_outbounds.json
{
    "outbounds":[
        {
            "protocol":"freedom",
...
EOF
	...
	# dns
	cat <<EOF >/etc/captain-claw/xray/conf/11_dns.json
{
    "dns": {
        "servers": [
          "localhost"
...
EOF
	...
	if echo "${selectCustomInstallType}" | grep -q 4 || [[ "$1" == "all" ]]; then
		...
		cat <<EOF >/etc/captain-claw/xray/conf/04_trojan_TCP_inbounds.json
{
"inbounds":[
	{
	  "port": 31296,
...
EOF
		addClients "/etc/captain-claw/xray/conf/04_trojan_TCP_inbounds.json" "${addClientsStatus}"
	fi
	...
	if echo "${selectCustomInstallType}" | grep -q 1 || [[ "$1" == "all" ]]; then
		fallbacksList=${fallbacksList}',{"path":"/'${customPath}'ws","dest":31297,"xver":1}'
		getClients "${configPath}../tmp/03_VLESS_WS_inbounds.json" "${addClientsStatus}"
		cat <<EOF >/etc/captain-claw/xray/conf/03_VLESS_WS_inbounds.json
{
"inbounds":[
    {
	  "port": 31297,
...
EOF
		addClients "/etc/captain-claw/xray/conf/03_VLESS_WS_inbounds.json" "${addClientsStatus}"
	fi
	...
	if echo "${selectCustomInstallType}" | grep -q 2 || [[ "$1" == "all" ]]; then
		...
		getClients "${configPath}../tmp/04_trojan_gRPC_inbounds.json" "${addClientsStatus}"
		cat <<EOF >/etc/captain-claw/xray/conf/04_trojan_gRPC_inbounds.json
{
    "inbounds": [
        {
            "port": 31304,
...
EOF
		addClients "/etc/captain-claw/xray/conf/04_trojan_gRPC_inbounds.json" "${addClientsStatus}"
	fi
	...
	if echo "${selectCustomInstallType}" | grep -q 3 || [[ "$1" == "all" ]]; then
		fallbacksList=${fallbacksList}',{"path":"/'${customPath}'vws","dest":31299,"xver":1}'
		getClients "${configPath}../tmp/05_VMess_WS_inbounds.json" "${addClientsStatus}"
		cat <<EOF >/etc/captain-claw/xray/conf/05_VMess_WS_inbounds.json
{
"inbounds":[
{
  "listen": "127.0.0.1",
...
EOF
		addClients "/etc/captain-claw/xray/conf/05_VMess_WS_inbounds.json" "${addClientsStatus}"
	fi

	if echo "${selectCustomInstallType}" | grep -q 5 || [[ "$1" == "all" ]]; then
		getClients "${configPath}../tmp/06_VLESS_gRPC_inbounds.json" "${addClientsStatus}"
		cat <<EOF >/etc/captain-claw/xray/conf/06_VLESS_gRPC_inbounds.json
{
    "inbounds":[
    {
        "port": 31301,
...
EOF
		addClients "/etc/captain-claw/xray/conf/06_VLESS_gRPC_inbounds.json" "${addClientsStatus}"
	fi
	...
	cat <<EOF >/etc/captain-claw/xray/conf/02_VLESS_TCP_inbounds.json
{
"inbounds":[
{
  "port": ${defaultPort},
...
EOF
	addClients "/etc/captain-claw/xray/conf/02_VLESS_TCP_inbounds.json" "${addClientsStatus}"
}
...
initTrojanGoConfig() {
	...
	cat <<EOF >/etc/captain-claw/trojan/config_full.json
{
    "run_type": "server",
    ...
    "log_level":3,
    "log_file":"/etc/captain-claw/trojan/trojan.log",
    "password": [
        "${uuid}"
    ],
    "dns":[
...
defaultBase64Code() {
	...
	if [[ "${type}" == "vlesstcp" ]]; then
		...
		if [[ "${coreInstallType}" == "1" ]] && echo "${currentInstallProtocolType}" | grep -q 0; then
			...
			cat <<EOF >>"/etc/captain-claw/subscribe_tmp/${subAccount}"
...
backupNginxConfig() {
	if [[ "$1" == "backup" ]]; then
		cp /etc/nginx/conf.d/alone.conf /etc/captain-claw/alone_backup.conf
		echoContent green " ---> nginx配置文件备份成功"
	fi

	if [[ "$1" == "restoreBackup" ]] && [[ -f "/etc/captain-claw/alone_backup.conf" ]]; then
		cp /etc/captain-claw/alone_backup.conf /etc/nginx/conf.d/alone.conf
		echoContent green " ---> nginx配置文件恢复备份成功"
		rm /etc/captain-claw/alone_backup.conf
	fi
...
unInstall() {
	...
	if [[ -d "/etc/captain-claw/tls" ]] && [[ -n $(find /etc/captain-claw/tls/ -name "*.key") ]] && [[ -n $(find /etc/captain-claw/tls/ -name "*.crt") ]]; then
		mv /etc/captain-claw/tls /tmp/v2ray-agent-tls
...
updateV2RayAgent() {
	...
	rm -rf /etc/captain-claw/install.sh
	if wget --help | grep -q show-progress; then
		wget -c -q --show-progress -P /etc/captain-claw/ -N --no-check-certificate "https://raw.githubusercontent.com/sh1375/v2ray-agent/master/install.sh"
	else
		wget -c -q -P /etc/captain-claw/ -N --no-check-certificate "https://raw.githubusercontent.com/sh1375/v2ray-agent/master/install.sh"
	fi

	sudo chmod 700 /etc/captain-claw/install.sh
	local version
	version=$(grep '当前版本:v' "/etc/captain-claw/install.sh" | awk -F "[v]" '{print $2}' | tail -n +2 | head -n 1 | awk -F "[\"]" '{print $1}')
...
EOF
	...
	3)
		...
		;;
	4)
		tail -n 100 /etc/captain-claw/crontab_tls.log
		;;
	5)
		tail -n 100 /etc/captain-claw/tls/acme.log
		;;
...
aliasInstall() {
	...
	if [[ -f "$HOME/install.sh" ]] && [[ -d "/etc/v2ray-agent" ]] && grep <"$HOME/install.sh" -q "作者:sh1375"; then
		mv "$HOME/install.sh" /etc/captain-claw/install.sh
		local roseType=
		if [[ -d "/usr/bin/" ]]; then
			if [[ ! -f "/usr/bin/rose" ]]; then
				ln -s /etc/captain-claw/install.sh /usr/bin/rose
			...
			if [[ ! -f "/usr/sbin/rose" ]]; then
				ln -s /etc/captain-claw/install.sh /usr/sbin/rose
...
unInstallHysteriaCore() {
	...
	handleHysteria stop
	rm -rf /etc/captain-claw/hysteria/*
	rm -rf /etc/systemd/system/hysteria.service
...
subscribe() {
	if [[ -n "${configPath}" ]]; then
		...
		rm -rf /etc/captain-claw/subscribe/*
		rm -rf /etc/captain-claw/subscribe_tmp/*
		showAccounts >/dev/null
		mv /etc/captain-claw/subscribe_tmp/* /etc/captain-claw/subscribe/

		if [[ -n $(ls /etc/captain-claw/subscribe/) ]]; then
			find /etc/captain-claw/subscribe/* | while read -r email; do
				email=$(echo "${email}" | awk -F "[b][e][/]" '{print $2}')

				local base64Result
				base64Result=$(base64 -w 0 "/etc/captain-claw/subscribe/${email}")
				echo "${base64Result}" >"/etc/captain-claw/subscribe/${email}"
				echoContent skyBlue "--------------------------------------------------------------"
