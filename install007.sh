...
# 检查2082、17500端口占用情况
checkPortUsedStatus() {
    if lsof -i tcp:2082 | grep -q LISTEN; then
        echoContent red "\n ---> 2082端口被占用，请手动关闭后安装\n"
        lsof -i tcp:2082 | grep LISTEN
        exit 0
    fi

    if lsof -i tcp:17500 | grep -q LISTEN; then
        echoContent red "\n ---> 17500端口被占用，请手动关闭后安装\n"
        lsof -i tcp:2082 | grep LISTEN
        exit 0
    fi
...
checkIP() {
    ...
    if [[ -z ${localIP} ]] || ! echo "${localIP}" | sed '1{s/[^(]*(//;s/).*//;q}' | grep -q '\.' && ! echo "${localIP}" | sed '1{s/[^(]*(//;s/).*//;q}' | grep -q ':'; then
        ...
        local portFirewallPortStatus="17500、2082"
...
customPortFunction() {
    ...
    if [[ -z "${currentPort}" && -z "${customPort}" ]] || [[ "${historyCustomPortStatus}" == "n" ]]; then
        echo
        echoContent yellow "请输入端口[默认: 17500]，如自定义端口，只允许使用DNS申请证书[回车使用默认]"
...
installTLS() {
    ...
    if [[ -f "/etc/v2ray-agent/tls/${tlsDomain}.crt" && -f "/etc/v2ray-agent/tls/${tlsDomain}.key" && -n $(cat "/etc/v2ray-agent/tls/${tlsDomain}.crt") ]] || [[ -d "$HOME/.acme.sh/${tlsDomain}_ecc" && -f "$HOME/.acme.sh/${tlsDomain}_ecc/${tlsDomain}.key" && -f "$HOME/.acme.sh/${tlsDomain}_ecc/${tlsDomain}.cer" ]]; then
        ...
        if [[ ! -f "/etc/v2ray-agent/tls/${tlsDomain}.crt" || ! -f "/etc/v2ray-agent/tls/${tlsDomain}.key" ]] || [[ -z $(cat "/etc/v2ray-agent/tls/${tlsDomain}.key") || -z $(cat "/etc/v2ray-agent/tls/${tlsDomain}.crt") ]]; then
            ...
            installTLSCount=1
            echo
            echoContent red " ---> TLS安装失败，正在检查2082、17500端口是否开放"
            allowPort 2082
            allowPort 17500
    ...
    cat <<EOF >/etc/v2ray-agent/hysteria/conf/config.json
{
	...
	"resolver": "https://8.8.8.8:17500/dns-query"
}
EOF
...
addCorePort() {
    ...
    if [[ "${selectNewPortType}" == "1" ]]; then
        ...
        read -r -p "请输入默认的端口号，同时会更改订阅端口以及节点端口，[回车]默认17500:" defaultPort
...
setDokodemoDoorUnblockStreamingMediaOutbounds() {
    ...
    if [[ -n "${setIP}" ]]; then
        ...
        unInstallOutbounds streamingMedia-2082
        unInstallOutbounds streamingMedia-17500

        outbounds=$(jq -r ".outbounds += [{\"tag\":\"streamingMedia-2082\",\"protocol\":\"freedom\",\"settings\":{\"domainStrategy\":\"AsIs\",\"redirect\":\"${setIP}:22387\"}},{\"tag\":\"streamingMedia-17500\",\"protocol\":\"freedom\",\"settings\":{\"domainStrategy\":\"AsIs\",\"redirect\":\"${setIP}:22388\"}}]" ${configPath}10_ipv4_outbounds.json)

        echo "${outbounds}" | jq . >${configPath}10_ipv4_outbounds.json

        if [[ -f "${configPath}09_routing.json" ]]; then
            unInstallRouting streamingMedia-2082 outboundTag
            unInstallRouting streamingMedia-17500 outboundTag

            local routing

            routing=$(jq -r ".routing.rules += [{\"type\":\"field\",\"port\":2082,\"domain\":[\"ip.sb\",\"geosite:${domainList//,/\",\"geosite:}\"],\"outboundTag\":\"streamingMedia-2082\"},{\"type\":\"field\",\"port\":17500,\"domain\":[\"ip.sb\",\"geosite:${domainList//,/\",\"geosite:}\"],\"outboundTag\":\"streamingMedia-17500\"}]" ${configPath}09_routing.json)
    ...
    "rules": [
      {
        ...
        "outboundTag": "streamingMedia-2082"
      },
      {
        "type": "field",
        "port": 17500,
        ...
        "outboundTag": "streamingMedia-17500"
      }
    ]
  }
}
...
EOF
        ...
        if [[ -f "${configPath}09_routing.json" ]]; then
            unInstallRouting streamingMedia-2082 inboundTag
            unInstallRouting streamingMedia-17500 inboundTag

            local routing
            routing=$(jq -r ".routing.rules += [{\"source\":[\"${setIPs//,/\",\"}\"],\"type\":\"field\",\"inboundTag\":[\"streamingMedia-2082\",\"streamingMedia-17500\"],\"outboundTag\":\"direct\"},{\"domains\":[\"geosite:${domainList//,/\",\"geosite:}\"],\"type\":\"field\",\"inboundTag\":[\"streamingMedia-2082\",\"streamingMedia-17500\"],\"outboundTag\":\"blackhole-out\"}]" ${configPath}09_routing.json)
            echo "${routing}" | jq . >${configPath}09_routing.json
        else
            cat <<EOF >${configPath}09_routing.json
            {
    ...
    if [[ -n "${configPath}" ]]; then
        ...
        if [[ -n $(ls /etc/v2ray-agent/subscribe/) ]]; then
            find /etc/v2ray-agent/subscribe/* | while read -r email; do
                ...
                if [[ -n "${currentDefaultPort}" && "${currentDefaultPort}" != "17500" ]]; then
